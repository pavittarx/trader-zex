...
class PEADStrategyConfig(StrategyConfig, frozen=True):
    # Defaults come from config.PEAD_* so the strategy, the live runner, and the
    # research scripts share one source of truth (see pead_core.py).
    instrument_id: str
    bar_type: str
    reaction_events: dict          # {ist_date_iso: reaction_float}
    hold_bars: int = config.PEAD_HOLD_BARS
    stop_pct: float = config.PEAD_STOP_PCT     # disaster stop (wide — preserve the drift)
    thresh: float = config.PEAD_THRESH         # |reaction| filter
    corp_gap: float = config.PEAD_CORP_GAP     # overnight gap above this = probable corp action
    alloc_pct: float = config.PEAD_ALLOC_PCT   # equity fraction per position (~3 concurrent)
    max_gross: float = config.PEAD_MAX_GROSS   # portfolio gross-exposure cap
