...
**Instrument validation (done now, no market data needed):** `momentum_ic.py --self-test` generates (a) a synthetic panel with a *planted* momentum effect and
...
uv run python -m strategies.momentum.research.momentum_ic --self-test

# live in-sample test (needs Fyers creds / reachable daily data)
uv run python -m strategies.momentum.research.momentum_ic --all-symbols --years 4
uv run python -m strategies.momentum.research.momentum_ic --symbols NSE:RELIANCE-EQ NSE:TCS-EQ ... --years 4
```
...
```bash
uv run python -m strategies.momentum.research.momentum_ic --github --universe allsymbols   # pre-registered 30
uv run python -m strategies.momentum.research.momentum_ic --github --universe top --top-n 200
uv run python -m strategies.momentum.research.momentum_ic --github --universe all          # full NIFTY500
```
...
position we'd have held. Implemented as `momentum_ic --pit-top-k K`:

```bash
uv run python -m strategies.momentum.research.momentum_ic --github --universe all --pit-top-k 200
```
...
top-200 L/S book (`momentum_ic --overlay`):
