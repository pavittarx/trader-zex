...
The cheap vectorized screen (mirrors strategies/pead/research/pead_event_ic.py):
  ...
  - Dollar-neutral L/S quintile spread (beta ~ 0), net of turnover cost  <- premium.
  - Long-only top quintile vs equal-weight "market": alpha / beta       <- is it just beta?
  - Point-in-time liquidity universe (--pit-top-k): kills survivorship.
  - Crash-risk overlays (--overlay): vol-scaling + market-trend filter.

Run:
  python -m strategies.momentum.research.momentum_ic --self-test
  python -m strategies.momentum.research.momentum_ic --github --universe all --pit-top-k 200 --overlay
  python -m strategies.momentum.research.momentum_ic --all-symbols --years 4   # live (Fyers)

--self-test runs the whole pipeline on synthetic panels (no market data): a
PLANTED-momentum panel (expect IC>0, t>2) and a pure-NOISE panel (expect IC~0).
Proves the measurement code has power and does not manufacture signal.
"""
...
from core.research import stats as rstats
...
# --------------------------------------------------------------------------- #
# Live data (Fyers, via the shared cached fetcher)
# --------------------------------------------------------------------------- #
def load_panel(symbols, years):
    """Wide (dates x symbols) close-price DataFrame from Fyers (cached)."""
    ...
    for s in symbols:
        ...
        if not df.empty and len(df) > LOOKBACK + SKIP + 21:
            c = df.sort_index()["close"]
...
_CACHE = os.path.expanduser("~/.trader_zex_nifty500_cache")
...
def pooled_ic(rec: pd.DataFrame) -> tuple[float, float, int]:
    ...
    across rebalances (the standard IC information-ratio, not a single-sample t)."""
    ics = []
...
def annualized(monthly: np.ndarray, ppy: int = 12):
    """(ann_ret, ann_vol, Sharpe, maxDD) from a per-period return series.
    Sharpe/maxDD via core.research.stats (shared across the pipeline)."""
    monthly = np.asarray(monthly, float)
    if len(monthly) == 0:
        return float("nan"), float("nan"), float("nan"), float("nan")
    ann_ret = monthly.mean() * ppy
    ann_vol = monthly.std() * np.sqrt(ppy) if len(monthly) > 1 else float("nan")
    return ann_ret, ann_vol, rstats.sharpe(monthly, periods=ppy), rstats.max_drawdown(monthly)
...
def report(rec: pd.DataFrame, cost_rt_bps: float = COST_RT_BPS, label: str = "") -> dict:
    ...
    cost = to * (cost_rt_bps / 1e4)   # replaced fraction pays a round trip
    ...
    print(f"  net hit rate (months up) : {float((ls_net > 0).mean())*100:5.1f}%")
    ...
    print(f"  H1 net ann.ret {h1.mean()*12*100:+6.1f}%  Sharpe {annualized(h1)[2]:+.2f}  (n={len(h1)})")
    print(f"  H2 net ann.ret {h2.mean()*12*100:+6.1f}%  Sharpe {annualized(h2)[2]:+.2f}  (n={len(h2)})")
...
def self_test() -> None:
    ...
    ra = report(build_records(_synth_panel(momentum=True, seed=1)), label="planted momentum")

    print("\n[B] PURE-NOISE panel — expect IC ~ 0, |t| small:")
    rb = report(build_records(_synth_panel(momentum=False, seed=2)), label="pure noise")
...
def main() -> None:
    ...
    p.add_argument("--all-symbols", action="store_true", help="use config.ALL_SYMBOLS")
    p.add_argument("--years", type=float, default=4.0)
    p.add_argument("--cost-bps", type=float, default=COST_RT_BPS)
    p.add_argument("--self-test", action="store_true",
                   help="synthetic-data instrument validation (no market data)")
    p.add_argument("--github", action="store_true",
                   help="use the public split-adjusted GitHub NIFTY500 dataset (2012-2021)")
    p.add_argument("--universe", choices=["allsymbols", "top", "all"],
                   default="allsymbols", help="--github universe selector")
    p.add_argument("--top-n", type=int, default=200, help="N names for --universe top")
    p.add_argument("--pit-top-k", type=int, default=0,
                   help="point-in-time universe: keep top-K names by trailing liquidity "
                        "at EACH rebalance (kills 'grew-into-index' survivorship). Needs --github.")
    p.add_argument("--pit-liq-window", type=int, default=63,
                   help="trailing days for the PIT liquidity median (default ~3mo)")
    p.add_argument("--overlay", action="store_true",
                   help="also run crash-risk overlays (vol-scaling + market-trend filter)")
    args = p.parse_args()
